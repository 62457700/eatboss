...
//請填入你申請的Line Messages Api資訊
//如果需要主動提醒的話，需要有Message push API的權限
//可以申請開發者試用的話免費的也有消息推送API權限，但有限制好友數量
var bot = linebot（{
...
bot.on（'message'，function（event）{
    ...
    if（text =='eatboss'|| text =='吃王小幫手'）{
        授權（sourceId，event，helpBoss）;
        返回;
    }

    if（text =='list'|| text =='清單'）{
        授權（sourceId，event，getList）;
        回歸;
    }


    if（text =='boss'|| text =='時間表'）{
        授權（sourceId，event，getBoss）;
        回歸;
    }
    ...
    if（（arr [0] =='distribution'|| arr [0] =='分配'）&& arr.length == 2）{
        event.postData = arr;
        授權（sourceId，event，distributionGift）;
    ...
    if（（arr [0] =='distribution'|| arr [0] =='分配'）&& arr.length> 2）{
        event.postData = arr;
        授權（sourceId，event，distributionGift2）;
    ...
    if（arr [0] =='checkin'|| arr [0] =='報到'）{
        if（arr.length == 2）{
            event.postData = arr;
            授權（sourceId，event，checkInMember）;
    ...
    if（arr [0] =='alert'|| arr [0] =='提醒'）{
        if（arr.length == 2 && Number.isInteger（Number.parseInt（arr [1]）））{
            event.postData = arr;
            授權（sourceId，event，setAlertTime）
        }

        if（arr.length == 1）{
            授權（sourceId，event，alertBoss）;
        }
    ...
    if（（arr [0] =='clear'|| arr [0] =='清除'）&& arr.length == 2）{
        event.postData = arr;
        授權（sourceId，event，clearBoss）;
        回歸;
    }

    if（arr [0] =='note'|| arr [0] =='備註'）{
        event.postData = arr;
        授權（sourceId，event，noteBoss）;
        回歸;
    }
    ...
    if（（arr [0] =='add'|| arr [0] =='加入'）
        ...
        event.postData = arr;
        授權（sourceId，event，addBoss）;
        回歸;
    }

    if（（arr [0] =='remove'|| arr [0] =='刪除'）&& arr.length == 2）{
        event.postData = arr;
        授權（sourceId，event，removeBoss）;
        回歸;
    }
...
app.use（function（err，req，res，next）{
  ...
  res.locals.error = req.app.get（'env'）==='development'？呃 ： {};
...
function helpBoss（event，clans）{
    var clan = clans [0];
    var message =“歡迎使用吃王小幫手！\ n \ n”;

    message + =“目前綁定伺服器：”+ clan.server +“\ n”;
    message + =“目前綁定血盟：”+ clan.name +“\ n”;
    message + =“作者：命運女神 ”+“\ n”;
    if（clan.alert_time）{
        message + =“目前提醒功能：開啟（提前”+ clan.alert_time +“分鐘提醒）\ n”;
    } else {
        message + =“目前提醒功能：關閉”
    }

    message + =“目前開放的指令有：boss（時間表），note（備註），alert（提醒），add（加入），remove（刪除），set（設定），clear（清除），list（清單） ，域名註冊\ n \ n“個;
    message + =“指令輸入中文或英文皆可。\ n \ n”;

    message + =“指令：更新時間表\ n”;
    message + =“說明：\ n”;
    message + =“輸入[編號] [死亡時間] \ n即可更新時間表。\ nex。70 0930代表編號70的頭目在九點三十分死亡。\ n \ n”;

    message + =“指令：boss \ n”;
    message + =“說明：\ n”;
    message + =“輸入boss或時間表，將顯示目前紀錄的所有時間表。\ n \ n”;

    message + =“指令：note \ n”;
    message + =“說明：\ n”;
    message + =“輸入\ nnote編號[文字] \ n或\ n備註編號[文字] \ n即可修改該頭目的備註訊息。\ n \ n”;

    message + =“指令：alert [分鐘]”;
    message + =“範例：\ n”;
    message + =“alert 15 \ n這樣代表每隻王重生前15分鐘會先提醒，設定為0代表關閉提醒功能。\ n \ n”;

    message + =“指令：alert \ n”;
    message + =“說明：\ n”;
    message + =“輸入提醒或提醒，將顯示最近N分鐘內會重生的頭目。\ n \ n”;

    message + =“指令：add \ n”;
    message + =“說明：\ n”;
    消息+ =“輸入\ nadd [編號] [修改頭目名稱] [修改重生間隔] [固定重生] \ n或\ n加入[編號] [修改頭目名稱] [修改重生間隔] [固定重生] \ n即可增加頭目到時間表內\ n固定重生時間非必填，填了只會在固定的時間提醒\ n \ n“個。

    message + =“指令：remove \ n”;
    message + =“說明：\ n”;
    message + =“輸入刪除編號\ n或\ n刪除編號\ n即可刪除該頭目。\ n \ n”;

    message + =“指令：set \ n”;
    message + =“說明：\ n”;
    message + =“輸入set編號[修改頭目名稱] [修改重生間隔] \ n或\ n設定編號[修改頭目名稱] [修改重生間隔] \ n更新頭目資訊。\ n \ n”;

    message + =“指令：clear \ n”;
    message + =“說明：\ n”;
    message + =“輸入清除[編號]，即可清除該編號的時間。\ n \ n”;

    message + =“指令：清單\ n”;
    message + =“說明：\ n”;
    message + =“輸入清單或列表，顯示目前所有戰利品排隊狀況。\ n \ n”;

    message + =“指令：清單增加\ n”;
    message + =“說明：\ n”;
    message + =“輸入清單增加[戰利品名稱]或list add [戰利品名稱]，即可增加戰利品種類。\ n \ n”;

    message + =“指令：清單刪除\ n”;
    message + =“說明：\ n”;
    message + =“輸入清單刪除[戰利品名稱]或list remove [戰利品名稱]，即可刪除戰利品種類。\ n \ n”;

    message + =“指令：排隊\ n”;
    message + =“說明：\ n”;
    message + =“輸入排隊[戰利品名稱]或陣容[戰利品名稱]，即可參加戰利品得排隊分配。\ n \ n”;

    message + =“指令：分配\ n”;
    message + =“說明：\ n”;
    message + =“輸入分配[戰利品名稱]或分發[戰利品名稱]，即可分配戰利品給最優先的人選\ n \ n”;

    message + =“指令：報到\ n”;
    message + =“說明：\ n”;
    message + =“輸入報到[遊戲ID]或checkin [遊戲ID]，即可完成ID登記\ n \ n”;

    message + =“指令：whois \ n”;
    message + =“說明：\ n”;
    message + =“輸入whois [遊戲ID]或whois [遊戲ID]，[遊戲ID]，....，即可查詢遊戲ID是誰\ n \ n”;
...
function getList（event，clans）{
    var clan = clans [0];
    con.query（'SELECT list。*，queue.line_id FROM list列表LEFT JOIN隊列在queue.list_id = list.id WHERE list.clan_id =？ORDER BY list.id，queue.id'，clan.id，function（err） ，列表）{
        if（！lists.length）{
...
function getBoss（event，clans）{
    ...
    con.query（'SELECT * FROM boss WHERE clan_id =？ORDER BY IF（TIMESTAMPDIFF（MINUTE，CONVERT_TZ（now（），“+ 00：00”，“+ 08：00”），estimated_time）> = 0，TIMESTAMPDIFF（ MINUTE，CONVERT_TZ（now（），“+ 00：00”，“+ 08：00”），estimated_time），999999999）'，clan.id，function（err，bosses）{
        for（var i = 0; i <bosses.length; i ++）{
            ...
            消息+ = boss.number +“\ t”
                + boss.name +“\ t”
                +（boss.estimated_time？小時+''+分鐘：''）+“\ t”
                +（boss.note？boss.note：''）+“\ n”
        }
...
function alertBoss（event，clans）{
    ...
    con.query（'SELECT *，TIMESTAMPDIFF（MINUTE，？，estimated_time）作為時間FROM boss WHERE clan_id =？AND estimated_time> =？AND estimated_time <=？ORDER BY estimated_time'，[
        日期，
        clan.id，
        日期，
        結束日期
        ...
        var message ='以下王將在'+ delayMinutes +“分鐘內重生：\ n”;
        for（var i = 0; i <rows.length; i ++）{
            var boss = rows [i];
            消息+ =“【提醒】”+ boss.number
                +“地圖的”+ boss.name
                +“將在”+ boss.time +“分鐘後重生\ n”;
        }
...
function pushBoss（clan）{
    ...
    con.query（'SELECT *，TIMESTAMPDIFF（MINUTE，？，estimated_time）作為時間FROM boss WHERE clan_id =？AND estimated_time> =？AND estimated_time <=？AND is_alert = 0 AND times IS NULL'，[
        日期，
        clan.id，
        日期，
        結束日期
        ...
        if（錯誤）{
            console.log（clan.name +'取得boss資料出錯！'）;
            回歸;
        }
        ...
        if（ids.length）{
            con.query（'UPDATE boss set is_alert = 1其中id in（？）'，[ids]，function（err，rows）{
                if（錯誤）{
                   的console.log（ERR）;
                }
            }）
...
function checkInMember（event，clans）{
    ...
    con.query（'UPDATE成員SET？WHERE line_id =？AND clan_id =？'，[
        {name：data [1]}，
        用戶身份，
        clan.id
    ]，function（err，rows）{
...
var j = schedule.scheduleJob（'* / 2 * * * *'，function（）{
    con.query（'SELECT * FROM clan WHERE line_id不為NULL AND alert_time> 0'，[]，function（err，clans）{
        if（錯誤）{
            的console.log（'定時推送錯誤：'）;
            的console.log（ERR）;
            回歸;
